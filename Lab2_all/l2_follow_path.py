    ...
    def follow_path(self):
        while not rospy.is_shutdown():
            ...
            for t in range(1, self.horizon_timesteps + 1):
                local_paths[t,:,0] = local_paths[t-1,:,0] + np.cos(local_paths[t-1,:,2]) * self.all_opts[:,0]
                local_paths[t,:,1] = local_paths[t-1,:,1] + np.sin(local_paths[t-1,:,2]) * self.all_opts[:,0]
                local_paths[t,:,2] = local_paths[t-1,:,2] + self.all_opts[:,1]
                pass

            # check all trajectory points for collisions
            # first find the closest collision point in the map to each local path point
            local_paths_pixels = (self.map_origin[:2] + local_paths[:, :, :2]) / self.map_resolution
            valid_opts = np.arange(self.num_opts)
            ...
            for opt in range(local_paths_pixels.shape[1]):
                for timestep in range(local_paths_pixels.shape[0]):
                    local_paths_lowest_collision_dist[opt] = (np.linalg.norm(local_paths_pixels[timestep][opt]-self.map_nonzero_idxes,axis=1)).min()
                    if local_paths_lowest_collision_dist[opt] < COLLISION_RADIUS:
                        valid_opts[opt] = -1

            # remove trajectories that were deemed to have collisions
            valid_ind = (valid_opts != -1)
            valid_opts = valid_opts[valid_ind]
            local_paths = local_paths[:,valid_ind,:]
            local_paths_lowest_collision_dist = local_paths_lowest_collision_dist[valid_ind]

            # calculate final cost and choose best option
            final_cost = np.linalg.norm(self.cur_goal-local_paths[self.horizon_timesteps],axis=1) + local_paths_lowest_collision_dist
