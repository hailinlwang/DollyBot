...
class wheelRadiusEstimator():
    ...
    def startStopCallback(self, msg):
        ...
        if self.isMoving is False and np.absolute(input_velocity_mag) > 0:
            ...
            # radius = 2*(x_true/sum_k(encoder right + encoder left))##
            # delta(phi_r(t)) = measured angle change over timestamp
            radius = 2 * DRIVEN_DISTANCE/(TICKS_PER_ROTATION*(self.del_left_encoder+self.del_right_encoder))
