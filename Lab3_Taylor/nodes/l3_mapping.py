...
class OccupancyGripMap:
    ...
    def scan_cb(self, scan_msg):
        ...
        x,y,theta = odom_map[0],odom_map[1],odom_map[2]
        for i in range(0,scan_msg.ranges,SCAN_DOWNSAMPLE):
            if scan_msg.ranges[i] < scan_msg.range_min or scan_msg.ranges[i] > scan_msg.range_max:
                continue
            theta += theta + scan_msg.angle_increment
            self.np_map, self.log_odds = self.ray_trace_update(self.np_map,self.log_odds,x,y,theta,scan_msg.ranges[i])
    ...
    def ray_trace_update(self, map, log_odds, x_start, y_start, angle, range_mes):
        ...
        # ray trace will give all pixels along ray
        # for each pixel, if closer than rk return small prob
        # if large k return prior probability
        # if approximately rk return above 0.5

        x_end = x_start + np.cos(angle)*range_mes
        y_end = y_start + np.sin(angle)*range_mes

        pix_x, pix_y = ray_trace(x_start,y_start,x_end,y_end)
